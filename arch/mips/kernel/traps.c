...
#include <linux/user_backtrace.h>
...
#undef DEBUG_USER_MMAP
#define DEBUG_ICACHE

#if defined(DEBUG_ICACHE)

// It's unclear why these aren't in <asm/mipsregs.h>
#define read_c0_datalo()         __read_32bit_c0_register($28, 1)
#define read_c0_datahi()         __read_32bit_c0_register($29, 1)

#define ICACHE_BUFFER_LINES 3

struct icache_buffer
{
	unsigned long start_address;
	unsigned long cache_lines[ICACHE_BUFFER_LINES][4];
	char cache_line_valid[ICACHE_BUFFER_LINES];
};

// inline so that we continue to run from uncached RAM.
static __always_inline unsigned long physical_address(unsigned long address)
{
	// If we're in KSEG0 or KSEG1 then this is easy
	if (address >= 0x80000000) {
		if (address >= 0xa0000000)
			return address - 0xa0000000;
		else
			return address - 0x80000000;
	} else if (!in_atomic_preempt_off()) {
		// This might schedule. :(
		...
		if (pgd) {
			pud_t *pud = pud_offset(pgd, page_addr);
			if (pud) {
				pmd_t *pmd = pmd_offset(pud, page_addr);
				if (pmd) {
					spinlock_t *ptl;
					pte_t *pte = pte_offset_map_lock(mm, pmd, page_addr, &ptl);
					if (pte) {
						if (pte_present(*pte))
...
static int get_icache_line_impl(unsigned long virt, unsigned long *buffer)
{
	static const unsigned long CACHE_SIZE = 16384; // CS
	static const unsigned long CACHE_WAYS = 2; // A
	static const unsigned long CACHE_LINE_SIZE = 16;
	static const unsigned long CACHE_INDEX_MASK = ((CACHE_SIZE / CACHE_WAYS) - 1);

	unsigned long phys = physical_address(virt);
	unsigned long cache_index = virt & CACHE_INDEX_MASK;
	unsigned long way;

	for(way = 0; way < CACHE_WAYS; ++way)
	{
		unsigned long tag_lo;
		unsigned long cached_address;
		
		// The index needs to be unmapped to avoid TLB exceptions
		cache_op(Index_Load_Tag_I, KSEG0 | (cache_index | (way * CACHE_SIZE / CACHE_WAYS)));		
		tag_lo = read_c0_taglo();
		
		if (tag_lo & (1<<7)) {
			cached_address = (tag_lo & ~CACHE_INDEX_MASK) | (phys & CACHE_INDEX_MASK);
			
			if (cached_address == phys) {
				unsigned int entry;
				for(entry = 0; entry < (CACHE_LINE_SIZE/4); ++entry) {
					cache_op(Index_Load_Tag_I, KSEG0 | (way * CACHE_SIZE / CACHE_WAYS) | (cache_index & ~(CACHE_LINE_SIZE - 1)) | (entry * 4));
					if (unlikely(tag_lo != read_c0_taglo()))
						printk("TAGLO MISMATCH for %lx at %u %lx vs %x\n", virt, entry, tag_lo, read_c0_taglo());

					buffer[entry] = read_c0_datalo();
				}
				return 1;
			}
		}
...
// Arrange to run get_icache_line_impl from uncached memory so we
// don't pollute the cache more than we have to.
static __always_inline int get_icache_line(unsigned long virt, unsigned long *buffer)
{
	int (*f)(unsigned long, unsigned long *) = &get_icache_line_impl;
	*((unsigned long *)(&f)) |= KSEG1;
	return f(virt, buffer);
}

void save_icache_near_epc_impl(const struct pt_regs *regs, struct icache_buffer *buffer)
{
	int line;
	unsigned long virt = (regs->cp0_epc & ~15) - (ICACHE_BUFFER_LINES - 1) * 16;

	unsigned long flags;
	local_irq_save(flags);
	buffer->start_address = virt;

	for(line = 0; line < ICACHE_BUFFER_LINES; ++line) {
		buffer->cache_line_valid[line] = get_icache_line(virt, buffer->cache_lines[line]);
		virt += 16;
	}
	local_irq_restore(flags);
}

// Arrange to run save_icache_near_epc_impl from uncached memory so we
// don't pollute the cache more than we have to.
static __always_inline void save_icache_near_epc(const struct pt_regs *regs, struct icache_buffer *buffer)
{
	void (*f)(const struct pt_regs *, struct icache_buffer *) = &save_icache_near_epc_impl;
	*((unsigned long *)(&f)) |= KSEG1;
	return f(regs, buffer);
}

void print_icache_buffer(const struct pt_regs *regs, const struct icache_buffer *buffer)
{
	int line;
	unsigned long address = buffer->start_address;

	for(line = 0; line < ICACHE_BUFFER_LINES; ++line) {
		printk("ICACHE %08lx:", address);
		if (buffer->cache_line_valid[line]) {
			int column;
			for(column = 0; column < 4; ++column) {
				if (address + column * 4 == regs->cp0_epc)
					printk("<%08lx>", buffer->cache_lines[line][column]);
				else
					printk(" %08lx ", buffer->cache_lines[line][column]);
			}
			printk("\n");
...
#define DIE_STACK_SIGNATURE 0xd1e57acc
static unsigned int die_stack_signature __attribute__ ((section (".bss_noinit")));
...
{
	if (die_stack_signature != DIE_STACK_SIGNATURE)
		return 0;
	
	if (*ppos >= sizeof(die_stack)) {
		// Make sure that we no longer look like we have a
		// stack trace available.
		die_stack_signature = 0;
		return 0;
	}
...
void __noreturn die(const char *str, struct pt_regs *regs)
{
	...
	int sig = SIGSEGV;
#if defined(DEBUG_ICACHE)
	struct icache_buffer icache_buffer;
#endif
...
#if defined(DEBUG_ICACHE)
	save_icache_near_epc(regs, &icache_buffer);
#endif // DEBUG_ICACHE
	...
	show_registers(regs);
#if defined(DEBUG_ICACHE)
	print_icache_buffer(regs, &icache_buffer);
#endif
...
#if defined(DEBUG_USER_MMAP)
void dump_map(struct task_struct *task)
{
	unsigned int i = 0;
	struct vm_area_struct *vma;
	struct mm_struct *mm = get_task_mm(task);
	if (!mm) {
		printk("Process %s has no MM\n", task->comm);
		return;
	}

	printk("Memory map for process %s(%u)\n", task->comm, strlen(task->comm));
	...
	for(i = 0; i < mm->map_count; ++i) {
		...
		printk("%08lx-%08lx %c%c%c%c %08lx %02x:%02x %-6lu ",
		       vma->vm_start,
		       vma->vm_end,
		       flags & VM_READ ? 'r' : '-',
		       flags & VM_WRITE ? 'w' : '-',
		...
		if (file) {
			char buffer[256];
			char *p = d_path(vma->vm_file->f_path.dentry, vma->vm_file->f_path.mnt,
...
#endif // DEBUG_USER_MMAP

#if defined(CONFIG_DEBUG_USER)
void user_backtrace_impl(int sig, struct pt_regs *regs, const char *file, int line)
{
#if defined(DEBUG_ICACHE)
	// This is a bit later than would be ideal but it's the best
	// we can do without being too intrusive. Push this up into
	// the actual handler if you aren't getting what you need.
	struct icache_buffer icache_buffer;
	save_icache_near_epc(regs, &icache_buffer);
#endif
...
#ifdef CONFIG_DEBUG_USER
		printk(KERN_ALERT "Signal %d in do_ri, epc=%08lx, ra=%08lx\n",
			   status, regs->cp0_epc, regs->regs[31]);
#endif
		regs->regs[31] = old31;
