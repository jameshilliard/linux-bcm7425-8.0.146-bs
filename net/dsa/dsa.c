		 ...
		 struct device *parent, struct device *host_dev)
{
	...
	newname[0] = 'p';
	rtnl_lock();
	memcpy (newname + 1, dst->master_netdev->name, IFNAMSIZ - 1);
	newname[IFNAMSIZ] = 0;
	netdev_info(dst->master_netdev, "%s: now known as %s\n", dst->master_netdev->name, newname);
	dev_change_name(dst->master_netdev, newname);
	rtnl_unlock();
