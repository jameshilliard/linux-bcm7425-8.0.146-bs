...
static DEFINE_MUTEX(devkmsg_output_sem);
...
static void devkmsg_console_emit(struct devkmsg_user *user, const char *buf, size_t count)
{
	...
	for(i = 0; i < count; ++i) {
		if (newline) {
			vfs_write(console_file, timecode, timecode_len, &console_file->f_pos);
			newline = 0;
		}
		 ...
		 */
		if (buf[i] == '\n' || buf[i] == '\r') {
			vfs_write(console_file, buf + prev, i - prev, &console_file->f_pos);
			if (buf[i] == '\n') {
				vfs_write(console_file, "\r\n", 2, &console_file->f_pos);
				newline = 1;
			}
			prev = i + 1;
		}
...
static int devkmsg_open(struct inode *inode, struct file *file)
{
	...
	/* write-only with no console does not need any file context */
	if (!console_file && ((file->f_flags & O_ACCMODE) == O_WRONLY)) {
