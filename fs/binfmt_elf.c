...
static u8 *ltcore_bitset_lookup_or_create(unsigned long pg)
{
	...
	if (!bitset) {
		bitset = get_zeroed_page(GFP_ATOMIC);
		if (bitset) {
...
{
	unsigned long seg;
	elf_addr_t base = auxv_lookup_addr(AT_BASE);
	...
	for(seg = 0; seg < phnum; ++seg) {
		...
		if (hdr.p_type == PT_DYNAMIC) {
			...
			if (hdr.p_memsz >= 131072) {
				pr_err("%s: Dynamic segment too large: %u\n", __func__, hdr.p_memsz);
				return;
			}

			/* Add the dynamic segment to the core dump so the linker can find r_debug */
			ltcore_add_range(hdr.p_vaddr, hdr.p_vaddr + hdr.p_memsz);
			...
			if (__copy_from_user(dyn, (const char __user *)hdr.p_vaddr, hdr.p_memsz)) {
...
#if defined(__mips__)
				/* On MIPS r_debug isn't stored
				 * directly in the .dynamic
				 * section. It can be found at the
				 * address pointed to by the
				 * DT_MIPS_RLD_MAP entry.
				 */
				if (dyn[i].d_tag == DT_MIPS_RLD_MAP) {
					const unsigned long mips_rld_map = dyn[i].d_un.d_ptr;
					pr_debug("%s: found mips_rld_map at 0x%x\n", __func__, mips_rld_map);
					ltcore_add_range(mips_rld_map, mips_rld_map + sizeof(unsigned long));

				        if (__copy_from_user(&r_debug, (const unsigned long __user *)mips_rld_map, sizeof(unsigned long)) == 0) {
						pr_debug("%s: found r_debug at 0x%x\n", __func__, mips_rld_map);
						break;
					}
					else
						pr_err("%s: Failed to read rld_map address\n", __func__);
				}
#else // !defined(__mips__)
				if (dyn[i].d_tag == DT_DEBUG) {
					pr_debug("%s: found r_debug at 0x%x\n", __func__, dyn[i].d_un.d_ptr);
