...
#include "rtk_bt_8821.h"
#include "rtk_misc_8821.h"

#define VERSION "3.1"

#ifdef BTCOEX
#include "rtk_coex_8821.h"
#endif
...
static struct usb_driver rtk_btusb_driver;
...
#if 1 || HCI_VERSION_CODE >= KERNEL_VERSION(3, 19, 0)
...
static void btusb_intr_complete(struct urb *urb)
{
	...
	if (urb->status == 0) {
		...
		rtk_btcoex_parse_event(urb->transfer_buffer,
				urb->actual_length);
#endif
#if 0 && HCI_VERSION_CODE < KERNEL_VERSION(3, 18, 0)
...
#if 0 && HCI_VERSION_CODE < KERNEL_VERSION(4, 1, 0)
	int i;
#endif

	/* When in kernel 4.4.0 and greater, the HCI_RUNNING bit is
	 * cleared in hci_dev_do_close(). */
#if 0 && HCI_VERSION_CODE < KERNEL_VERSION(4, 4, 0)
		       ...
		       const struct usb_device_id *id)
{
	...
	if (data->isoc) {
		err = usb_driver_claim_interface(&rtk_btusb_driver,
						 data->isoc, data);
		if (err < 0) {
			hci_free_dev(hdev);
			rtk_free(data);
...
static void btusb_disconnect(struct usb_interface *intf)
{
	...
	if (intf == data->isoc)
		usb_driver_release_interface(&rtk_btusb_driver, data->intf);
	else if (data->isoc)
		usb_driver_release_interface(&rtk_btusb_driver, data->isoc);
...
static int __init rtk_btusb_init(void)
{
	RTKBT_DBG("Realtek Bluetooth USB driver ver %s", VERSION);
#ifdef BTCOEX
	rtk_btcoex_init();
#endif
	return usb_register(&rtk_btusb_driver);
}

static void __exit rtk_btusb_exit(void)
{
	RTKBT_DBG("rtk_btusb: btusb_exit");
	usb_deregister(&rtk_btusb_driver);
...
module_init(rtk_btusb_init);
module_exit(rtk_btusb_exit);

MODULE_AUTHOR("");
MODULE_DESCRIPTION("Realtek Bluetooth USB (BrightSign) driver ver " VERSION);
