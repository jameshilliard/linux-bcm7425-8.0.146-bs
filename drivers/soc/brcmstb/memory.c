...
#ifdef CONFIG_BRIGHTSIGN_MEM_RESERVE
/* Reads the memory reservation for the given region number and invokes
 * the reservation callback so long as the region type (CMA/BMEM) matches
 * the callback region type.
 * Regions numbers are contiguous and start from zero. If a region configuation
 * is not found then 1 is returned to indicate the end of reserved regions.
 * Otherwise, zero is returned for success or < 0 if a failure occurs.
 */
static int brightsign_populate_mem_reserve(const void *fdt, int dt_offset,
		unsigned region_nr, int addr_cells, int size_cells, bool cma,
		int (*setup)(phys_addr_t start, phys_addr_t size))
{
	...
	if (region_nr > 99) {
		pr_err("%s: too many regions\n", __func__);
		return -EINVAL;
	}

	snprintf(name, sizeof(name), "region%u", region_nr);
	dt_offset = fdt_subnode_offset(fdt, dt_offset, name);

	prop = fdt_get_property(fdt, dt_offset, "reg", &proplen);
	if (!prop)
		return 1;
	reg = prop->data;

	start = of_read_number(reg, addr_cells);
	reg += addr_cells;
	size = of_read_number(reg, size_cells);

	/* We must _never_ put BMEM in the first 256M because the
	   rescue kernel will want to be able to use that memory
	   without risking it being locked out due to it having been
	   used as a secure heap at some point in the past.
	*/
	if (unlikely(size > 0 && (start < 256UL << 20))) {
		pr_err("CMA/BMEM not permitted within first 256MiB\n");
		/* We don't BUG() here because printk hasn't been
		   initialised yet and early_printk isn't enabled by
		   default. Let's continue booting but stop reserving
		   memory so that this gets noticed later.
		*/
		return -EINVAL;
	}

	prop = fdt_get_property(fdt, dt_offset, "cma", &proplen);
	cma_region = (prop != NULL);

	/* If the region type matches the type requested for the
	 * callback then invoke the callback.
	 */
	if (cma == cma_region) {
		pr_err("%s [%d]: %llu MiB @ %pa\n", cma_region ? "CMA" : "BMEM",
				region_nr, size >> 20, &start);
		setup(start, size);
	}
	return 0;
}

static int __init brightsign_memory_reserve(bool cma,
		int (*setup)(phys_addr_t start, phys_addr_t size))
{
	const void *fdt = initial_boot_params;
	const struct fdt_property *prop;
	int dt_offset;
	int regions_dt_offset;
	int addr_cells = 1, size_cells = 1;
	int proplen;
	char name[32];
	struct memblock_type *type = &memblock.memory;
	struct memblock_region *region = &type->regions[0];
	int megabytes = 0;
	unsigned region_nr = 0;
	int rc;

	/* If there is a reserved memory configuration defined in the FDT
	 * under /brightsign/nexus-memory then select that, otherwise, log an
	 * error but don't reserve any memory and continue booting.
	 */
	dt_offset = fdt_path_offset(fdt, "/brightsign");
	if (dt_offset >= 0)
		dt_offset = fdt_subnode_offset(fdt, dt_offset, "nexus-memory");

	if (dt_offset < 0) {
		pr_err("WARNING: BRIGHTSIGN NEXUS MEMORY IS NOT CONFIGURED.\n");
		return -EINVAL;
	}
	...
	if (unlikely(addr_cells < 0 || addr_cells > 2))
		return -EINVAL;

	if (unlikely(size_cells < 0 || size_cells > 2))
		return -EINVAL;

	/* tiger7444 and tiger 7252 have a common device tree but
	   require different memory configurations. The megabytes
	   node allows us to have different memory maps depending on the amount
	   of physical memory available.
	   */
	if (type->cnt > 0)
		megabytes = region->size >> 20;

	if (unlikely(megabytes < 256)) {
		pr_err("Unsupported memory configuration: base %pa size %pa\n",
				&region->base, &region->size);
		return -EINVAL;
	}

	// Support reading a value for a specific amount of memory.
	// This is used for Tiger which has a single device tree for
	// multiple memory configurations.
	snprintf(name, sizeof(name), "megabytes_0x%x", megabytes);
	regions_dt_offset = fdt_subnode_offset(fdt, dt_offset, name);

	// But if we don't find one, then the regions should just be
	// listed in the parent node.
	if (regions_dt_offset < 0)
		regions_dt_offset = dt_offset;

	do
	{
		rc = brightsign_populate_mem_reserve(fdt, regions_dt_offset,
				region_nr, addr_cells, size_cells, cma, setup);
		region_nr++;
	} while (rc == 0);
...
void __init brightsign_memory_bmem_reserve(int (*setup)(phys_addr_t start,
							phys_addr_t size))
{
	if (brightsign_memory_reserve(false, setup) < 0)
		pr_err("Failed to configure Brightsign BMEM memory\n");
}

void __init brightsign_memory_cma_reserve(int (*setup)(phys_addr_t start,
							phys_addr_t size))
{
	if (brightsign_memory_reserve(true, setup) < 0)
		pr_err("Failed to configure Brightsign CMA memory\n");
}
