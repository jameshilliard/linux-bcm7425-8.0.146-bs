	...
	if (Channel <= 14) {
		...
		if (1  <= Channel && Channel <= 2)
			...
			*pGroup = 4;
			...
			RTW_INFO("==>mpt_GetChnlGroup8812A in 2.4 G, but Channel %d in Group not found\n", Channel);
	} else {
		bIn24G = _FALSE;
		...
		if (36   <= Channel && Channel <=  42)
			...
			*pGroup = 13;
			...
			RTW_INFO("==>mpt_GetChnlGroup8812A in 5G, but Channel %d in Group not found\n", Channel);

	}
	/* RTW_INFO("<==mpt_GetChnlGroup8812A,  (%s) Channel = %d, Group =%d,\n", (bIn24G) ? "2.4G" : "5G", Channel, *pGroup); */
...
)
{
	...
	/* RTW_INFO("hal_ReadPowerValueFromPROM8812A(): PROMContent[0x%x]=0x%x\n", (eeAddr+1), PROMContent[eeAddr+1]); */

	if (AutoLoadFail) {
		RTW_INFO("hal_ReadPowerValueFromPROM8812A(): Use Default value!\n");
	...
	for (rfPath = 0 ; rfPath < MAX_RF_PATH ; rfPath++) {
		...
		for (group = 0 ; group < MAX_CHNL_GROUP_24G; group++) {
			...
			/* RTW_INFO("8812-2G RF-%d-G-%d CCK-Addr-%x BASE=%x\n",  */
		...
		for (group = 0 ; group < MAX_CHNL_GROUP_24G - 1; group++) {
			...
			if (pwrInfo24G->IndexBW40_Base[rfPath][group] == 0xFF)
				pwrInfo24G->IndexBW40_Base[rfPath][group] =	EEPROM_DEFAULT_24G_INDEX;
			/* RTW_INFO("8812-2G RF-%d-G-%d BW40-Addr-%x BASE=%x\n",  */
		...
		for (TxCount = 0; TxCount < MAX_TX_COUNT; TxCount++) {
			if (TxCount == 0) {
				...
				/*RTW_INFO("8812-2G RF-%d-SS-%d BW20-Addr-%x DIFF=%d\n",
				...
				/*RTW_INFO("8812-2G RF-%d-SS-%d LGOD-Addr-%x DIFF=%d\n",
				...
				/*RTW_INFO("8812-2G RF-%d-SS-%d BW40-Addr-%x DIFF=%d\n",
				...
				/*RTW_INFO("8812-2G RF-%d-SS-%d CCK-Addr-%x DIFF=%d\n",
		...
		for (group = 0 ; group < MAX_CHNL_GROUP_5G; group++) {
			...
			/* RTW_INFO("8812-5G RF-%d-G-%d BW40-Addr-%x BASE=%x\n",  */
		...
		for (TxCount = 0; TxCount < MAX_TX_COUNT; TxCount++) {
			if (TxCount == 0) {
				...
				/*RTW_INFO("8812-5G RF-%d-SS-%d BW20-Addr-%x DIFF=%d\n",
				...
				/*RTW_INFO("8812-5G RF-%d-SS-%d LGOD-Addr-%x DIFF=%d\n",
				...
				/*RTW_INFO("8812-5G RF-%d-SS-%d BW40-Addr-%x DIFF=%d\n",
		...
		for (TxCount = 0; TxCount < MAX_TX_COUNT; TxCount++) {
			...
			/*RTW_INFO("8812-5G RF-%d-SS-%d BW80-Addr-%x DIFF=%d\n",
			...
			/*RTW_INFO("8812-5G RF-%d-SS-%d BW160-Addr-%x DIFF=%d\n",
...
#if 1
	phydm_UpdateHalRAMask(&pHalData->odmpriv, psta->wireless_mode, pHalData->rf_type, psta->bw_mode, MimoPs_enable, disable_cck_rate, &ratr_bitmap_msb, &mask, rssi_level);
#else
