...
void update_bmc_sta(_adapter *padapter)
{
	...
	if (psta) {
		...
		mdelay(1);	// BrightSign #26312 - AP mode does not start reliably without this delay
...
#define DUMP_ADAPTERS_STATUS 0
	...
	/* BrightSign bug 26267: It turns out that wpa_supplicant in
	   AP mode always puts a WPS IE in when proto=RSN. If we trust
	   the check above then we'll never start the BSS and there
	   will be no beacons. In the absence of advice from Broadcom,
	   let's just start the BSS always. This function only seems
	   to be called once with wpa_supplicant anyway.
	*/
	RTW_INFO("Starting BSS..");
	pmlmeext->bstart_bss = _TRUE;
...
void stop_ap_mode(_adapter *padapter)
{
	...
	/* BrightSign Bug#27001 - replace broadcast/multicast STA info structure */
	/* (otherwise all broadcast packets get dropped after exiting AP mode) */
	rtw_init_bcmc_stainfo(padapter);
