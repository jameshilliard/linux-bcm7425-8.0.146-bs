...
_func_enter_;
	...
	/* BrightSign: disable IRQ before enabling device, may have pending interrupt */
...
static int rtw_drv_init(struct pci_dev *pdev, const struct pci_device_id *pdid)
{
	...
	/* BrightSign: interrupt is disabled from device enable until irq allocation complete */
	enable_irq(pdev->irq);
