...
static u8 pci_write_port_not_xmitframe(void *d,  u32 size, u8 *pBuf, u8 qsel)
{
...
#ifdef CONFIG_64BIT_DMA 
	SET_TX_BD_PHYSICAL_ADDR0_HIGH(txbd, (mapping>>32) & 0xFFFFFFFF);
#endif
	...
	SET_TXBUFFER_DESC_ADD_HIGH_WITH_OFFSET(txbd, 1,
				      ((mapping + TX_WIFI_INFO_SIZE) >> 32) & 0xFFFFFFFF); /* pkt */
#endif
...
#ifdef CONFIG_64BIT_DMA	
       /* We make a test allocation to attempt to discover the DMA address size
	* For RTL8821AE, this was done using the Rx descriptor mapping, but we
	* need to know earlier on RTL8821CE since we use DMA to load the firmware.
	* This approach is dubious, but is equivalent to the RTL8821AE driver.
	*
	* The correct solution is to use the bdma64 flag, which is set based
	* on whether the platform reports 64-bit DMA capability.  However, we
	* seem to have a problem on some 64-bit platforms which was hidden by
	* the previous method.
	*
	* The 128 byte DMA test allocation size is arbitrary.
	*/

	struct pci_dev *pdev = adapter_to_dvobj(padapter)->ppcidev;
	u64 test_dma;
	u8* testbd = pci_alloc_consistent(pdev, 128, &test_dma);
	if ((test_dma >> 32) == 0)
	{
		printk(KERN_ERR "rtl8821ce: 64-bit platform using 32-bit DMA\n");
		adapter_to_dvobj(padapter)->bdma64 = _FALSE;
	}
	pci_free_consistent(pdev, 128, testbd, test_dma);
