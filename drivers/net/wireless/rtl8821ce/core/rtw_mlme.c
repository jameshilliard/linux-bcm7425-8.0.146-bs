...
void rtw_sta_mstatus_report(_adapter *adapter)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) && check_fwstate(pmlmepriv, WIFI_ASOC_STATE)) {
		...
		if (psta)
			...
			/*
			 * BrightSign: we get here during shutdown/reboot, via cfg80211_rtw_disconnect().  BS#30700
			 * The warning is potentially alarming, and of little help to anyone, so let's suppress it.
			 */
			/* rtw_warn_on(1); */
		}
	}
