...
static void bcmgenet_enable_leds(struct net_device *dev,
				 bool enable)
{
	struct bcmgenet_priv *priv = netdev_priv(dev);
	const u32 lnk_ovrd_en = (1<<15);
	const u32 lnk_status_ovrd = (1<<14);

	/* GENETv5 drives the link and activity LEDs differently. We
	   also happen to have inverted the polarity of the LEDs in
	   hardware. Unfortunately this means that when the interface
	   is closed, the activity LED comes on. So, let's make sure
	   we artificially override the link status to cause the LED
	   to go off before GENET is turned off, and then stop
	   overriding it when it is turned back on again. */
	if (GENET_IS_V5(priv)) {
		u32 reg = bcmgenet_sys_readl(priv, LED_CNTRL);
		if (enable) {
			/* Activity LED under GENET control */
			reg &= ~lnk_ovrd_en;
		} else {
			/* Activity LED forced off */
			reg |= lnk_ovrd_en;
		}
		/* Link status down - this is the default, but it
		 * doesn't do any harm. */
		reg &= ~lnk_status_ovrd;
		bcmgenet_sys_writel(priv, reg, LED_CNTRL);
	}
}
					   ...
					   struct enet_cb *cb)
{
	...
	if (skb && dma_unmap_addr(cb, dma_addr)) {
		dma_unmap_single(dev, dma_unmap_addr(cb, dma_addr),
...
static netdev_tx_t bcmgenet_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (ring->free_bds <= (nr_frags + 1)) {
		if (!netif_tx_queue_stopped(txq)) {
			...
			ci = bcmgenet_tdma_ring_readl(priv, ring->index, TDMA_CONS_INDEX);
			netdev_err(dev, "%s: RP %02x%08x BD %04x CI %04x PI %04x SZ %08x\n",
				   __func__,
				   bcmgenet_tdma_ring_readl(priv, ring->index, TDMA_READ_PTR_HI) & 0xff,
				   bcmgenet_tdma_ring_readl(priv, ring->index, TDMA_READ_PTR),
				   (ci >> 16), ci & 0xffff,
				   bcmgenet_tdma_ring_readl(priv, ring->index, TDMA_PROD_INDEX),
				   bcmgenet_tdma_ring_readl(priv, ring->index, DMA_RING_BUF_SIZE));
					  ...
					  struct enet_cb *cb)
{
	...
	/* Grab the current Rx skb from the ring and DMA-unmap it if required*/
	rx_skb = bcmgenet_free_rx_cb(kdev, cb);

	/* Put the new Rx skb (or NULL) on the ring */
	cb->skb = skb;
	dma_unmap_addr_set(cb, dma_addr, mapping);
	dma_unmap_len_set(cb, dma_len, priv->rx_buf_len);
	dmadesc_set_addr(priv, cb->bd_addr, mapping);

        /* Return the current Rx skb to the caller, which may be NULL if we were
         * OOM when allocating.  In that case we will have placed the oom_dma_addr
         * mapping and a NULL skb on the ring.
         */
	return rx_skb;
}
...
static int bcmgenet_close(struct net_device *dev)
{
	...
	bcmgenet_enable_leds(dev, false);
	if (priv->internal_phy)
...
static int bcmgenet_probe(struct platform_device *pdev)
{
	...
	if (dn) {
		const void *macaddr = of_get_mac_address(dn);
	 ...
	 */
	priv->oom_skb = netdev_alloc_skb(priv->dev, RX_BUF_LENGTH);
	if (!priv->oom_skb) {
		dev_err(&pdev->dev, "failed to allocate spare SKB\n");
		err = -ENOMEM;
		goto err;
	}
	priv->oom_dma_addr = dma_map_single(&pdev->dev, priv->oom_skb->data,
				 RX_BUF_LENGTH, DMA_FROM_DEVICE);
	if (dma_mapping_error(&pdev->dev, priv->oom_dma_addr)) {
