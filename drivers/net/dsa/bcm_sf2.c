...
static int bcm_sf2_sw_setup(struct dsa_switch *ds)
{
	...
	/* BrightSign: use flooding instead of switch forwarding.  BS#32468
	 * We have had problems with traffic disruption using the built-in
	 * switch, which are avoided if we keep the forwarding database
	 * empty and rely on flooding of unknown packets.  Do this before
	 * the database is populated.  Disable learning on all ports.
	 */
	{
		...
		core_writel(priv, port_mask, CORE_DIS_LEARN);

		/* Clear out any old entries */
		for (port = 0; port < priv->hw_params.num_ports; port++)
			bcm_sf2_sw_fast_age_port(ds, port);

		pr_warn("sf2: flooding mode\n");
	}
	...
	if (priv->type == BCM7445) {
		/* BrightSign Tiger: Make the activity LED work the way we want. */
		reg_writel(priv, 0x02035403 | SPDLNK_SRC_SEL, REG_LED_0_CNTRL);
	}

	/* BrightSign: Don't drop LLDP packets. */
	reg = core_readl (priv, CORE_RSV_MCAST_CTRL);
	reg &= ~(EN_MUL_0 | EN_MUL_1);
	core_writel(priv, reg, CORE_RSV_MCAST_CTRL);
