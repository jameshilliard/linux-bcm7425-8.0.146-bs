...
extern int disable_device_mode;		// In phy-brcm-usb.c

/* Work-around for BCM7278 errata 7278-ER300.  BrightSign BS#31054 / Broadcom CS5375451 */
#define SOFT_DRD
#ifdef SOFT_DRD
 #include <linux/kthread.h>
 #include <linux/err.h>
 static struct task_struct *drdmon_task;
 static int drdmon_thread(void *);
 #define   PRESCALER_MASK	0x00ff0000
#endif
...
static void usb_init_common(struct brcm_usb_init_params *params)
{
	...
	if (USB_CTRL_MASK_FAMILY(params, USB_DEVICE_CTL1, PORT_MODE)) {
		...
		reg &= ~USB_CTRL_MASK_FAMILY(params, USB_DEVICE_CTL1,
					PORT_MODE);
#ifdef SOFT_DRD
		/* Errata on BCM7278 only, enable at run-time */
		if (params->selected_family == BRCM_FAMILY_7278A0) {
			/* if DRD, leave it in default (host) mode for now */
			if (params->device_mode < 2)
	...
	if (USB_CTRL_MASK_FAMILY(params, USB_PM, BDC_SOFT_RESETB)) {
		...
		case USB_CTLR_DEVICE_OFF:
			...
			// If we forced the device off, leave the BDC block on
			// so that we don't crash if we try and load the BDC driver
			if (disable_device_mode)
...
#ifdef SOFT_DRD
	/* Workaround for Errata 7278-ER300 */
	if (!drdmon_task && (params->selected_family == BRCM_FAMILY_7278A0)) {
		drdmon_task = kthread_run(drdmon_thread, params, "drdmon");
		if(IS_ERR(drdmon_task))
			printk("Cannot create drdmon task!\n");
		else
			printk("Soft DRD monitor started\n");
	}
#endif
}
...
#ifdef SOFT_DRD
#define USB_CTRL_USB_DRD_STATUS	0x5c
#define USB_DRD_STATUS_IDOUT	0x10
static int drdmon_thread(void *data)
{
	struct brcm_usb_init_params *p = (struct brcm_usb_init_params *) data;
	void __iomem *ctrl = p->ctrl_regs;
	u32 reg, next, prev = 0;
	int sleep_time = 500;	/* mS */

	while(1) {
		reg = brcmusb_readl(USB_CTRL_REG(ctrl, USB_DRD_STATUS));
		next = (reg & USB_DRD_STATUS_IDOUT) ? 1 : 0;
		if (disable_device_mode)
			next = 0;
		if (next != prev) {
			printk(KERN_INFO "Setting DRD state %d\n", next);
			brcm_usb_init_set_dual_select(p, next);
