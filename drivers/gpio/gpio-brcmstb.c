...
static int brcmstb_gpio_irq_set_type(struct irq_data *d, unsigned int type)
{
	...
	u32 level_and_edge_config, ilevel_and_edge_config;
	u32 level, ilevel;
	unsigned long flags;
	...
	case IRQ_TYPE_LEVEL_LOW:
		level = mask;
		level_and_edge_config = 0;
		edge_insensitive = 0;
		break;
	case IRQ_TYPE_LEVEL_HIGH:
		level = mask;
		level_and_edge_config = mask;
		edge_insensitive = 0;
		break;
	...
	case IRQ_TYPE_EDGE_BOTH:
		level = 0;
		level_and_edge_config = 0;  /* don't care, but want known value */
	...
	ilevel_and_edge_config = bank->bgc.read_reg(priv->reg_base +
			GIO_EC(bank->id)) & ~mask;
	...
	bank->bgc.write_reg(priv->reg_base + GIO_EC(bank->id),
			ilevel_and_edge_config | level_and_edge_config);
