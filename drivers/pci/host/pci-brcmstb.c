...
static int brcm_pcie_setup_bridge(struct brcm_pcie *pcie)
{
	...
	if (is_pcie_link_up(pcie, false)) {
		...
		/* We have observed that some PCIe devices generate a
		 * "imprecise external abort" during PCIe startup.  The effect
		 * varies between examples of apparently identical boards.
		 * It is suspected that the problem is triggered when the
		 * config request is retried, due to the peripheral not being
		 * quite ready.  To avoid this, add an extra 100ms delay before
		 * trying to use the device.  See BrightSign bug #25155.
		 */
		mdelay(100);
	}
